...
impl<'a> Client<'a> {
    ...
    pub(crate) fn resolve_trunk(&self) -> Result<String, Box<dyn Error>> {
        let trunk = self
            .jj
    ...
    pub fn submit(&self, remote: &str) -> Result<(), Box<dyn Error>> {
        ...
        // `---` separator goes only on the current PR (`@`, last in the
        // root-first chain) body, not every PR in the stack.
        if let Some((_, num, title, body)) = chain.last() {
...
mod tests {
    ...
    fn submit_two_commit_stack_chains_bases_and_drafts_deps() {
        ...
        // `---` is written only to the current PR (`@`), not every PR.
        let edits = find_calls(&cl, "gh", &["pr", "edit"]);
        assert_eq!(edits.len(), 1, "separator written to current PR only");
        assert!(
            edits
                .iter()
                .any(|e| e.1.iter().any(|x| x.contains("---\n"))),
            "current PR body gets the separator"
        );
    ...
    fn submit_formats_pr_description_with_dependency_graph() {
        ...
        assert!(
            body.ends_with("---\n"),
            "separator is appended at the end of the body"
        );
    }
    ...
    fn submit_resubmit_updates_existing_prs() {
        ...
        let edits = find_calls(&cl, "gh", &["pr", "edit"]);
        // Each existing PR is upsert-edited once; the `---` separator is
        // appended in a single extra edit on the current PR (`@` = #8) only.
        assert_eq!(edits.len(), 3, "2 upserts + 1 separator edit");
        assert!(
        ...
        // Exactly one edit carries the `---` separator, on the current PR.
        assert_eq!(
            edits
                .iter()
                .filter(|c| c.1.iter().any(|x| x.contains("---\n")))
                .count(),
            1,
            "separator written once, to the current PR"
        );
    }
